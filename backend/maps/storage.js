...
function resolveStorageConfig(options = {}) {
    ...
    return {
        mapsDir: options.mapsDir || runtimeResolved.mapsDir,
...
function createMapStorage(options = {}) {
    ...
    function legacySaveMap(mapDoc) {
        ...
        const nextMaps = [summary, ...withoutCurrent]
    ...
    function bootstrapSqliteFromLegacy({ force = false } = {}) {
        ...
        if (sqliteJobs.count() === 0) {
            ...
            if (legacyJobs.length > 0) sqliteJobs.replaceAll(legacyJobs);
        }
    ...
    function saveIndex(index) {
        ...
        if (config.exportCompatJson) legacySaveIndex(nextIndex);
        return nextIndex;
    }
    ...
    function saveMap(mapDoc) {
        ...
        if (config.exportCompatJson) legacySaveMap(mapDoc);
        return summary;
    }
    ...
    function promoteMap(mapId) {
        ...
        if (summary && config.exportCompatJson) {
            const mapDoc = sqliteMaps.getMap(mapId);
            if (mapDoc) legacySaveMap(mapDoc);
    ...
    function saveJobs(jobs) {
        ...
        if (config.exportCompatJson) legacySaveJobs(sorted);
        return sorted;
    }
...
function createMapStorageFromRuntimeFlags({
    runtimeFlags = {},
    env
} = {}) {
...
const defaultStorage = createMapStorageFromRuntimeFlags();

module.exports = defaultStorage;
module.exports.createMapStorage = createMapStorage;
module.exports.createMapStorageFromRuntimeFlags = createMapStorageFromRuntimeFlags;
