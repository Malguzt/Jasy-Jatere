...
- Phase 7: in progress with map job persistence in metadata store, map queue inputs sourced from repository-backed camera and observation metadata, mapper-first fallback execution (no backend local fallback path in runtime), detector `/events` fallback retired from runtime map generation, centralized legacy JSON adapter wiring for map storage/corrections compatibility paths (`maps/legacy-json-adapter`), runtime legacy-read bootstrap retired in favor of explicit migration bootstrap entrypoints, and map queue/storage/corrections modules refactored away from import-time singleton state to explicit composition-time factory instantiation.
...
- Phase 10: in progress with import-script test coverage for legacy JSON migration into repository-backed SQLite state, plus dedicated composition-factory tests for control-plane and stream-gateway service graphs, and runtime-default retirement of legacy metadata fallback/dual-write paths for camera/recording repositories (legacy compatibility writes now explicit opt-in only).
...
- Detector-local catalog artifacts (`recordings-index.json` and `*.meta.json`) are retired from runtime ownership; control-plane catalog APIs are the active source of truth.
