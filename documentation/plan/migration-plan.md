...
- Phase 3: in progress with worker-facing internal config APIs (`/api/internal/config/*`), detector camera-config and retention-policy consumption through control-plane snapshots, strict no-shared-file mode via `REQUIRE_CONTROL_PLANE_CAMERA_CONFIG`, and backend connectivity monitor legacy `cameras.json` fallback now gated behind `LEGACY_COMPAT_EXPORTS_ENABLED`.
