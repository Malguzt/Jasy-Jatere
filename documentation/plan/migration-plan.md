...
- Phase 7: in progress with map job persistence in metadata store, map queue inputs sourced from repository-backed camera and observation metadata, mapper-first fallback execution (no backend local fallback path in runtime), detector `/events` fallback retired from runtime map generation, and centralized legacy JSON adapter wiring for map storage/corrections compatibility paths (`maps/legacy-json-adapter`).
