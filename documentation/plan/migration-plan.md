...
- Phase 5: in progress with perception ingest and control-plane-owned recording catalog APIs (`/api/perception/*`, `/api/recordings`), retirement of detector recording aliases (`/api/detector/recordings*` -> `410`), detector `/recordings*` compatibility routes now proxying list/delete through control-plane catalog APIs (local catalog fallback retired), strict detector fallback control via `REQUIRE_CONTROL_PLANE_RECORDING_CATALOG`, and sidecar/index metadata writes limited to non-strict compatibility mode.
