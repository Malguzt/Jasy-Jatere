...
- Phase 4A: in progress with stream orchestration extracted into stream control services and WS gateway modules, plus lifecycle toggles (`STREAM_RUNTIME_ENABLED`, `STREAM_WEBSOCKET_GATEWAY_ENABLED`), transport capability negotiation (`GET /api/streams/capabilities`, `STREAM_WEBRTC_ENABLED`, `STREAM_WEBRTC_REQUIRE_HTTPS`), proxy support (`STREAM_GATEWAY_API_URL`) to support gradual runtime separation, compose-backed proxy mode via `make run-with-stream-gateway`, and proxy-aware readiness checks (`STREAM_PROXY_MODE_ENABLED`, `STREAM_PROXY_REQUIRED`).
